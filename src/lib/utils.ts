 ...
 *
 * - If the `"--abort-exits"` environment option is `false` throw a `TaskError`.
 ...
 */
export function updateFile(filename: string, find: RegExp, replace: string): boolean {
 ...
 * Valid glob patterns are those supported by the Deno `path` library.
	...
	if (Deno.build.os === 'windows') {
		...
		if (!shellExe) {
			...
			if (!existsSync(shellExe)) {
				abort(`cannot locate shell: no SHELL environment variable or ${shellExe} executable`);
			}
		}
